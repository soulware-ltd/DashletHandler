...
#dashletbox_container td  {
    padding: 3px;
    text-align: left;
}
#dashletbox_container td:nth-child(2){
    text-align: right;
}
</style>
{/literal}
...
<table width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top: 5px;">
 	<tr>
		...
		<td align='right'>
            {if !$lock_homepage}
            <a href="javascript: void(0)" id="DasletBox">{$mod_strings.LBL_DASHLET_SETTINGS}</a>
            <table id="dashletbox_container" style="text-align: right; display: none">
                <tr>
                    <td colspan="2"><input id="add_dashlets" class="button" type="button" value="{$lblAddDashlets}" onclick="return SUGAR.mySugar.showDashletsDialog();"/></td></tr>
                    {if $saved_dashlets}
                        <tr>
                            <td>
                                {$mod_strings.LBL_LOAD_DASHLET_SETTINGS}
                            </td>
                            <td>
                                <form class="inline" method="post" action="index.php?module=Home&action=dashlets_handler">

                                    <select name="saved_dashlets">
                                        {foreach from=$saved_dashlets item=dashlet}
                                            <option value="{$dashlet.id}">{$dashlet.title}</option>
                                        {/foreach}
                                    </select>
                                    <input type="hidden" name="method" value="loadFrom" />
                                    <input type="submit" value="{$mod_strings.LBL_LOAD}" name="loadDashlets" />
                                    {*<input type="submit" value="Delete" name="deleteDashlets" />*}
                                </form>
                            </td>
                        </tr>
                    {/if}
                <tr>
                    <td>
                        {$mod_strings.LBL_SAVE_DEFAULT_DASHLET_SETTINGS}
                    </td>
                    <td>
                        <form class="inline" method="post" action="index.php?module=Home&action=dashlets_handler">
                            <input type="text" name="title" />
                            <input type="hidden" name="method" value="saveAs" />
                            <input name="saveDashletAs" class="button" type="submit" value="{$mod_strings.LBL_SAVE}" />
                        </form>
                    </td>
                </tr>

                <tr><td>
                        <form class="inline" method="post" action="index.php?module=Home&action=dashlets_handler&m=load">
                            <input id="load_dashlets" name="loadDashlet" class="button" type="submit" value="{$lblLoadDashlets}" />
                            <input type="hidden" name="method" value="load" />
                        </form>
                        </td><td>
                        <form class="inline" method="post" action="index.php?module=Home&action=dashlets_handler&m=save">
                            <input id="save_dashlets" name="saveDashlet" class="button" name="saveDashlet" type="submit" value="{$lblSaveDashlets}" />
            ...
            <script type="text/javascript">
                {literal}
                $('#DasletBox').click(function(){
                    $('#dashletbox_container').fadeToggle(100);
                });
                {/literal}
            </script>
            {/if}
